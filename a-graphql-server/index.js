...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      let result = books;
      if (args.author) {
        result = result.reduce((array, book) => {
      ...
      if (args.genre) {
        result = result.reduce((array, book) => {
          if (book.genres.includes(args.genre.toLowerCase())) {
            array.push(book);
          }
          return array;
        }, []);
      ...
      return authors.map((author) => {
        return {
          ...
          bookCount: books.reduce(
            (prev, book) => prev + (book.author === author.name ? 1 : 0),
            0
          ),
        };
      });
  ...
  Mutation: {
    ...
    editAuthor: (root, args) => {
      ...
      const bookCount = books.reduce(
        (prev, book) => prev + (book.author === args.name ? 1 : 0),
        0
      );
